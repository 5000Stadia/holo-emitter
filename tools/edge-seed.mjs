...
`;
  ...
  if (style.same_wall && style.true_shape_recreate) {
    return `**Image 1 IS THE WARP'S OWN OUTPUT for this very view** — \`${style.file}\`, copied from ` +
      ...
      `per-axis correction, and every zone the warp's fill had smeared BLURRED IN PLACE (the revealed ` +
      `mask decides where) - full-frame, no margins, nothing to expand into. The prompt asks: keep every line of architecture and the ` +
      `framing exactly (corners and cornice may not move outward), repaint the blurred areas sharp, redraw every OBJECT in its true shape (a circle stays a circle) [true-shape ` +
