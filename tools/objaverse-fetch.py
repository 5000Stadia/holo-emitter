...
def main():
    ...
    for u in uids:
        ...
        if not any(lic.startswith(k) for k in OK_LIC): continue
        text = ((m.get("name") or "") + " " + (m.get("description") or "") + " " + " ".join(t.get("name", "") for t in (m.get("tags") or []))).lower()
        scored.append((-sum(w in text for w in words), u, m.get("name"), lic))
    scored.sort()
    if not scored:
    ...
    py = sys.executable
    gr = subprocess.run([py, os.path.join(ROOT, "tools", "mesh-ground.py"), glb, os.path.join(out, "model.glb"), "--height-m", str(a.height_m), "--up", "+y", "--level", a.level], capture_output=True, text=True)
    ...
    rec = {"schema": "library-record/mesh/0.1", "id": a.id, "noun": a.category, "dims_m": {"h": a.height_m, "w": a.width_m, "d": a.depth_m},
           "model": {"kind": "glb", "up": "+y", "grounded": bool(ground.get("ok")), "front": "+z", "source_faces": ground.get("faces")},
