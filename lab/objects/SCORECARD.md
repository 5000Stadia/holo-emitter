...
| 3 | TripoSR on CPU (single image → mesh) | ~65 min of install fights (no CUDA: marching-cubes shim; a GL import; trimesh vs numpy 2) | **~2 min** per asset on this CPU (22 s model + 40 s marching cubes + export; the first run's 18 min was weight download and first-run compile) | 1.5 MB raw (75,744 tris, vertex colours, no texture); decimated to 24k for the page | 0 | `tools/mesh-gate.py` (shape err 0.32 of 0.35 allowed: the blob is chunkier than the declared chair) | (pending the look) | |
