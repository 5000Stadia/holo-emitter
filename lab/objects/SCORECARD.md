...
| 3 | TripoSR on CPU (single image → mesh) | ~65 min of install fights (no CUDA: marching-cubes shim; a GL import; trimesh vs numpy 2) | **18 min** inference per asset on this CPU (seconds on any GPU); second run in progress after the export fix | (pending, expect a few MB GLB) | 0 | `tools/mesh-gate.py` | (pending) | |
