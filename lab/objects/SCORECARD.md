...
**Recommendation for the pipeline**, pending Kabe's grades: rooms from the plan (0); bulk furniture as primitives or Infinigen-style parametric families (C / 6); pieces that must match a picture through TripoSR then PrimitiveAnything (B → 5) with a real GPU box or hosted model when the look matters; img2threejs (A) only for objects that need named parts and motion. No billboards.
