...
- **A foot rule reads the whole licence, and reads the LOWEST step.** `skirting-base` first searched down from `pick_floor`'s minimum, which is taken inside the 17 px floor WINDOW; a deep facing's far wall painted 10 px nearer than the plan has its foot above that window, the minimum falls on the carpet and the "strongest step under it" is a carpet row (saloon/N 79247e99: foot 693 for a foot painted at 624, +56 % focal, and the wrong pair scales drove the learn step the wrong way). The window is where the plan EXPECTS the foot; the licence is where a reading is still a reading. Inside it, a chrome trim on the skirting's top steps brighter than the skirting's own foot, so the foot is the lowest dark-over-light step, not the strongest.
- **A guide's seams stop at a doorway.** The deep guide's far seams (cornice, capping strip, foot) are ruled lines, and a ruled line across a void is an object the painter copies: saloon_n/S carried the chrome strip across door02 on both pair-1 rolls, and the door reader (rightly) found no door under a bar at 1.06 m. Every line a guide draws is broken over every opening the far painting was promoted with — measured columns, carried through the same mapping that placed the wall.
