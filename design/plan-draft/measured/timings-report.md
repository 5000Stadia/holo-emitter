...
- 2026-08-24 21:30 tick — nominal: services up, loop pass age 39 s, nothing owed. `supersede.wall` 2.55× is the route now carrying the snap (~9 s) and the room audit per wall; one-time per roll, and the master bedchamber's four went through as a set in one pass. `emit.facing` flag is sub-second. Faster while keeping quality: the room audit re-reads every facing of the room per supersede — caching band features per (png sha) would make a four-wall set cost one audit, not four.
- 2026-08-25 tick — nominal: services up, loop pass age 44 s, one sub-second flag. The g5 batch (18 rolls) shows 0 returns ~25 min after the order: the seat spent that time on `git pull` and "reading the required project guidance" (CLAUDE.md sends every reader through method/intention/playbook/blueprint) before its first generation. Faster while keeping quality: a seat-scoped AGENTS.md at backdrops/ that says "you paint; read PACKET.md and nothing else" would cut that ~20 min preamble to ~1 min per session — a bigger single cut than anything in the loop, since the seat restarts several times a day.
