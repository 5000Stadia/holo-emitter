...
document; the CLI and the bake print them; `design/plan-draft/projection.md` §6 carries them.
...
**Law (b) has a witness for the first time.** It used to be held by the heavy black outline being
checkable *by eye*. Now: an `open` facing may have no built structure between its standpoint and
its far line, nor any standing on that line; a facing of an outdoor space that claims a wall must
have an exterior or garden band standing on the line it views; and every edge of the outline must
be the outer face of an exterior band, because §4b makes the outline the single source of every
outdoor wall and two hand-typed statements of one geometry drift. Only exterior and garden bands
count as structure an outdoor space can see — the garden wall is the one further built structure
Kabe ruled in on 2026-08-21.
...
`projection.md` §5 carries it to Kabe.
...
**`corner_x1_px − corner_x0_px = wall_width_m × px_per_m_at_wall`, both corners inside the
canvas** — the same statement wherever the wall does fill the frame. Written into the blueprint
beside the clause it replaces.
...
validator on its first run; recorded in `projection.md` §7.
