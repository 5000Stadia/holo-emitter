...
| B-RUNWALL | run-wall instrument support (single-corner landmark reading: translation from the one in-frame corner, scale from the ruled anchor; off-frame side unpinned) — capped builder on `run-wall-instruments`; the four refused run-wall paintings are its proof set | building |
| B-PLAYBOOK | the facing-situation playbook (Kabe: instruction sets per direction and room-piece type, composed as overlays, skipped when absent): `situationsOf` + per-pack report + PACKET/manifest stamping + `design/playbook-facings.md` — capped builder on `facing-playbook` | building |
