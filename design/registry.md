...
| handle | task | state |
|---|---|---|
| Q-DOOR-EDGE | queued (after B-DELIVERY): door apertures as the INSIDE EDGE of the frame — corners joined into a polygon, arched heads as contours; the void's dark run is evidence not the aperture (Kabe's blue/pink example) | queued |
