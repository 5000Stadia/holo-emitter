...
| B-DEEPVIEW | deterministic deep-view assembler (far painting pinhole-scaled + homography side planes from the cells' own paintings) — capped builder on branch `deep-view-derived`, running as the comparison to the painted same-wall route | building |
