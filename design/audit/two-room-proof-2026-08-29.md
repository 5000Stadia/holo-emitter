...
- Reveal fill landed (B-FILL, 27 turns): a revealed pixel keeps its surface and its across-parameter and is clamped only in depth along that surface's own receding line; cross-fade runs down the recession. back_office/S: the chevron is gone; the extended band is a directional streak — it goes the way the room goes.
- Exposed: a promoted wall is never re-routed — the sweep stands on the store. Re-warping the eight under the new fill meant deleting the store frames by hand. `build packs/<name> --rebuild` (clear the pack's store + exit state, run the chain) is the missing command.
