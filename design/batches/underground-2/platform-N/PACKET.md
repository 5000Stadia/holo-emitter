...
**Image 2 is this wall's edge seed.** `edge-seed-right.png` is the 10 % of `backdrops/platform/E.png` that abuts this picture — its left-hand 154 columns, full frame height, cut by `tools/crop-edge-seed.py` (sha256 `361a4ccb50ca` from a painting at `057f53711c30`). The prompt names its role in words: _Image 2 is a reference of exactly what sits at this picture's right edge - the scene continues from it seamlessly._
