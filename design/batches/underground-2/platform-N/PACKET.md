...
**Image 2 is this wall's edge seed.** `edge-seed-left.png` is the 10 % of `backdrops/platform/W.png` that abuts this picture — its right-hand 154 columns, full frame height, cut by `tools/crop-edge-seed.py` (sha256 `e4e2161bc825` from a painting at `89d66c7e0b22`). The prompt names its role in words: _Image 2 is a reference of exactly what sits at this picture's left edge - the scene continues from it seamlessly._
