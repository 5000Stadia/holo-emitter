...
**Image 1 IS THE WARP'S OWN OUTPUT for this very view** — `true-shape-platform_far-W.png`, copied from `backdrops/source-warped/platform_far-W/warped.png`: architecture geometry-exact and full-frame, objects ovalled by the per-axis correction. The prompt inverts the usual ask: keep every line of architecture and the framing exactly, redraw every OBJECT in its true shape (a circle stays a circle) [true-shape recreate, 2026-08-30 — the painter re-normalizes shapes unless ordered to copy, and a full-frame reference leaves nothing to zoom into].
