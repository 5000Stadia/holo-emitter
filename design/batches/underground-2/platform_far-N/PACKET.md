...
> **This ask WAITS for `platform_far/W`. Do not paint it until that wall's picture is on disk.**
>
> [row 42] Every location paints one wall first — its LEAD, the most-carried wall — and the other three are painted with the lead's own picture in front of the painter, which is what makes the four read as one room. This facing continues `platform_far/W`, which has no picture yet.
