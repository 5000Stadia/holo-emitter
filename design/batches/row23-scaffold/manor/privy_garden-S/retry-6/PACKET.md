# privy_garden/S — CONSISTENCY RE-ASK 6 (technique t2, labelled scaffold)

> **Why this wall is being asked again**
>
> This room is ruled to ONE set of materials and this facing does not show them. In PRIVY GARDEN, every wall is a garden wall of weathered red brick in English bond on a coursed stone plinth, open sky above it; every floor is raked gravel paths between low clipped box and turf. Paint exactly those materials on every surface in this view and nothing else — not a different wall lining, not a different ceiling, not a different floor. Measured, in the ASKS rather than in the pixels: every promoted facing of this room was commissioned from ONE set of materials, and that set is not the one this plan rules for the room. The materials above are the plan's ruling.
...
**No edge seed rides with this ask.** `privy_garden/E` — that neighbour is not painted; `privy_garden/W` — that neighbour is not painted. The materials are named in words in the prompt instead, and the words are the room's own ruling out of `tools/room-voices.mjs` — not another wall's pixels.
...
| roll 1 | `backdrops/source/privy_garden-S/row23-d27746b0.png` |

The prompt files are already on disk beside them. Do not rewrite them.

This wall: 189.5 px per metre at the wall plane, door + window.
Voice: **outdoors_walled** (room id); gate anchor **the stone string-course above the ground**, 0.95 m.
The promoted painting this replaces is still at `backdrops/privy_garden/S.png` and is not overwritten by this packet; promotion is the sweep's decision, not this emitter's.
