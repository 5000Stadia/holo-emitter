...
| `06-back_stair_head-W-flight-down` | the same flight from the landing above | going down, the steps fall below the frame within a metre. **The caption here said "what is left is the well they open in the floor" — that is false of this picture.** `well_poly` is empty for a descending flight by construction and nothing draws a well; what you are looking at is the flight's two stringer masses lying on the floor with the floor grid running between them, and a hole is not painted brighter than the floor round it. Round four's critic found it. Kabe: this frame and `04` are the two I would look at hardest |
